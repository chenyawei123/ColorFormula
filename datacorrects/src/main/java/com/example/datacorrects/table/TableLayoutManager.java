...
public class TableLayoutManager extends RecyclerView.LayoutManager {
			...
			for (int column = columnStart; column <= columnEnd; column++) {
				...
				if (!skip && Rect.intersects(mLayoutState.mSlideAreaRect, itemRect)) {
					...
					if (mBuild.mFixHeader) {
						...
						Log.i("TAG",layoutParams.height+"hhhhh");
					} else {
	 ...
	 * 获取指定行前面的高度
	 */
	public int getPreRowHeight(int row) {
		int preRowHeight = 0;
	 ...
	 * 获取显示列开始的位置
	 */
	public int getDisplayColumnStart() {
		int columnPre = 0;
	...
	public static class Build {
		...
		public Build(Context context) {
			mColumnCount = 1;
			mFixHeader = false;
			mHeadHeight = DensityUtil.dip2px(48) ;
			mRowHeight = DensityUtil.dip2px(20);
			mFixColumnCount = 0;
			mWidgetCount = 0;
		}
